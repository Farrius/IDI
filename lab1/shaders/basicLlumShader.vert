...
{	
    ...
    vec3 Lv = normalize(vec3(view*vec4(posFocus, 1.0) - view*TG*vec4(vertex, 1.0)));
    vec3 NormSCOv = normalize(NormalMatrix*normal);
    fcolor = Ambient() + Difus(NormSCOv,Lv,colorFocus) + Especular(NormSCOv, Lv, view*TG*vec4(vertex, 1.0), colorFocus);
